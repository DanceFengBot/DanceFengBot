...
public class UserRatioAP30Image {
...
//        }
        ...
        try {
            ...
            CompletableFuture<BufferedImage> backgroundImgFuture = CompletableFuture.supplyAsync(() -> {
                try {
                    // TODO 背景图片写到常量区（内存）
                    return ImageIO.read(new File(path + "Background3.png"));
                } catch(IOException e) {
                    throw new RuntimeException(e);
                }
            });
...
//            CompletableFuture.allOf(avatarFuture, boxFuture, titleFuture).join();
        ...
        List<RankMusicInfo> AP30List = RatioCalculator.getSubAP30List(allRankList, true);
        ...
        for(int row = 0; row<10; row++) { //列
            for(int col = 0; col<3; col++, index++) { //行
                if(index>=AP30List.size()) break out;

                int dx2 = col * dx;
                int dy2 = row * dy;
                RankMusicInfo musicInfo = AP30List.get(index);
        ...
        float avg1 = RatioCalculator.average(AP30List);
