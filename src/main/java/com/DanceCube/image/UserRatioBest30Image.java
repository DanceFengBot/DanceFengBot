...
public class UserRatioBest30Image {
...
//        }
        ...
        try {
            ...
            CompletableFuture<BufferedImage> backgroundImgFuture = CompletableFuture.supplyAsync(() -> {
                try {
                    // TODO 背景图片写到常量区（内存）
                    return ImageIO.read(new File(path + "Background2.png"));
                } catch(IOException e) {
                    throw new RuntimeException(e);
                }
            });
...
//            CompletableFuture.allOf(avatarFuture, boxFuture, titleFuture).join();
        ...
        List<RankMusicInfo> rank30List = RatioCalculator.getSubRank30List(allRankList, true);
        ...
        for(int row = 0; row<10; row++) { //列
            for(int col = 0; col<3; col++, index++) { //行
                if(index>=rank30List.size()) break out;

                int dx2 = col * dx;
                int dy2 = row * dy;
                RankMusicInfo musicInfo = rank30List.get(index);
        ...
        float avg1 = RatioCalculator.average(rank30List);
        String extraInfoText = """
                上次战力：%d
                B-30 战力：%.4f
                """.formatted(lvRatioHistory.getRatio(),
                avg1) + getRatioComment(lvRatio);
